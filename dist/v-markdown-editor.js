...
(function (mod) {
  ...
  CodeMirror.defineMode("markdown", function (cmCfg, modeCfg) {
    ...
    function blockNormal(stream, state) {
      ...
      if (state.indentationDiff >= 999999 && (prevLineIsIndentedCode || state.prevLine.fencedCodeEnd || state.prevLine.header || prevLineLineIsEmpty)) {
        stream.skipToEnd();
    ...
    function inlineNormal(stream, state) {
      ...
      if (ch === "~") {
        ...
        var leftFlanking = !/\s/.test(after) && (!punctuation.test(after) || /\s/.test(before) || punctuation.test(before));
        var rightFlanking = !/\s/.test(before) && (!punctuation.test(before) || /\s/.test(after) || punctuation.test(after));
        var setStrikethrough = null;
        
        if (!state.strikethrough && leftFlanking && (ch === "~" || !rightFlanking || punctuation.test(before))) setStrikethrough = true; else if (state.strikethrough == ch && rightFlanking && (ch === "~" || !leftFlanking || punctuation.test(after))) setStrikethrough = false;
        
        if (setStrikethrough != null) {
          if (setStrikethrough === true) state.strikethrough = ch;
          var t = getType(state);
          if (setStrikethrough === false) state.strikethrough = false;
          return t;
        }
        else if (ch === ' ') {
          if (stream.eat('~')) {
...
//# sourceMappingURL=v-markdown-editor.js.map